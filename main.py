...
# === Multi-Seed Ensemble Stability with tqdm ===
for seed in tqdm(SEEDS, desc="🔄 Processing seeds"):
    ...
    for col in tqdm(target_cols, desc=f"⚡ Processing targets for seed {seed}"):
        ...
        for fold, (train_idx, valid_idx) in enumerate(tqdm(kf.split(X_full), total=N_FOLDS, desc=f"🌿 KFold {col}"), 1):
            ...
            def xgb_objective(trial):
                params = {
                    ...
                    'reg_lambda': trial.suggest_loguniform('reg_lambda', 1e-3, 20.0),
                    'random_state': seed,
                    'eval_metric': 'mae'   # ✅ pass here instead of in fit
                }
            ...
            # Live progress bar for Optuna tuning
            ...
            model_lgbm = LGBMRegressor(**study_lgbm.best_params, random_state=seed, verbosity=-1)
        ...
        preds_seed.append(preds_final)

    preds_seed_stacked = np.vstack(preds_seed).T
    final_preds_all_seeds.append(preds_seed_stacked)
    gc.collect()

# === Final Advanced Weighted Ensembling ===
if USE_ERROR_WEIGHTING:
    weights = []
    for preds_seed_stacked in final_preds_all_seeds:
        errors = np.abs(preds_seed_stacked.mean(axis=1) - y_full.mean(axis=1).values[:len(preds_seed_stacked)])
        weights.append(1 / (errors + 1e-8))

    weights = np.array(weights)
    weights = weights / weights.sum(axis=0)
    final_preds = sum(weights[i][:, None] * final_preds_all_seeds[i] for i in range(len(SEEDS)))
else:
    final_preds = np.mean(final_preds_all_seeds, axis=0)

submission[target_cols] = final_preds
submission.to_csv("submission_shellAI_blend_optuna_tqdm_final.csv", index=False)
print("\n✅ Submission saved: submission_shellAI_blend_optuna_tqdm_final.csv")